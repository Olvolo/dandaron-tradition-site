...
class ArticleSectionController extends Controller
{
    ...
    public function store(Request $request, Article $article): RedirectResponse
    {
        ...
        $validated['slug'] = Str::slug($validated['title']);
        $validated['category_id'] = $article->category_id;

        /** @var Article $section */
        $section = $article->children()->create($validated);
        $section->authors()->sync($article->authors->pluck('id'));
    ...
    public function edit(Article $section): View
    {
        $section->load('parent');
        $sectionTree = $section->parent->getSectionTree($section->getKey());
        // Также передаём категории и авторов для формы
        $categories = Category::all();
        $authors = Author::all();
        return view('admin.articlesections.edit', compact('section', 'sectionTree', 'categories', 'authors'));
    }
    ...
    public function update(Request $request, Article $section): RedirectResponse
    {
        $validated = $request->validate([
            ...
            'order_column' => 'nullable|integer',
            'parent_id' => 'nullable|exists:articles,id',
            // Разделы не меняют категорию и авторов, они наследуют их от родителя
        ]);
